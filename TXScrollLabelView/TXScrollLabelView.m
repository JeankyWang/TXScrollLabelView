...
@property (assign, nonatomic) BOOL isAttri;
...
- (void)startup {
    ...
    if (_scrollType == TXScrollLabelViewTypeFlipRepeat || _scrollType == TXScrollLabelViewTypeFlipNoRepeat) {
        _firstTime = YES;
        if (_isAttri) { //初次显示
            self.upLabel.attributedText = [self.scrollArray firstObject];
            self.downLabel.attributedText = [self.scrollArray firstObject];
        } else {
...
-(NSArray *)getSeparatedLinesFromLabel {
    ...
    for (id line in lines) {
        ...
        NSString *lineString = [[text substringWithRange:range] stringByReplacingOccurrencesOfString:@"\n" withString:@""];
        if (_isAttri) {
            NSMutableAttributedString *attri = [[NSMutableAttributedString alloc] initWithString:lineString attributes:@{NSFontAttributeName:_font,NSForegroundColorAttributeName:_scrollTitleColor}];

            for (NSString *word in _keywords) {
                if ([lineString containsString:word]) {
                    [attri addAttributes:@{NSFontAttributeName:_font,NSForegroundColorAttributeName:_keywordColor} range:[lineString rangeOfString:word]];
                }
            }
            [linesArray addObject:attri];
        } else {
